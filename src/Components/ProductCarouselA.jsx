        ...
        {products.map((product) => (
          <SwiperSlide key={product.id}>
            <div className="bg-white p-6 shadow-lg rounded-lg text-center flex flex-col items-center">
              ...
              <p className="text-gray-600 text-md font-semibold  mt-1">{product.price}</p>
              <div className="flex justify-center items-center mt-2">
                {Array.from({ length: 5 }).map((_, index) => (
                  <Star
                    key={index}
                    className={`h-4 w-4 font-semibold ${index < Math.floor(product.rating) ? "text-yellow-500" : "text-gray-300"}`}
                  />
                ))}
                <span className="text-sm text-gray-500 font-semibold ml-2">({product.rating})</span>
              </div>
            </div>
            {/* <button></button> */}
          </SwiperSlide>
